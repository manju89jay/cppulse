...
ProjectConfig parse_yaml(const std::string& content) {
    ...
    while (std::getline(stream, line)) {
        ...
        if (section == Section::kExcludePaths && trimmed.size() > 2 && trimmed[0] == '-') {
...
ProjectConfig parse_json(const std::string& content) {
    ...
    if (doc.contains("rules") && doc["rules"].is_object()) {
        for (const auto& [rule_id, props] : doc["rules"].items()) {
            ...
            if (props.contains("warning_threshold") &&
                props["warning_threshold"].is_number_integer()) {
...
ProjectConfig load_config(const std::filesystem::path& config_path) {
    ...
    spdlog::info("Config loaded: profile={}, exclude_paths={}, rule_overrides={}", config.profile,
                 config.exclude_paths.size(), config.rules.size());
    return config;
}
...
bool matches_exclude_pattern(const std::filesystem::path& relative_path,
